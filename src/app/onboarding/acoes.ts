...
export async function gravarCompromisso(entrada: {
  ...
  const { error } = await supabase.from("commitments").insert({
    user_id: usuario.id,
    commitment_declaration: entrada.commitmentDeclaration.trim(),
    due_at: new Date(Date.now() + HORIZONTE_DO_COMPROMISSO_MS).toISOString(),
    information_nature: NATUREZA_DECLARADA,
    // `outcome` fica nulo: o resultado só existe depois do prazo, e entra
    // como linha nova (Q-10), nunca por UPDATE.
  });
